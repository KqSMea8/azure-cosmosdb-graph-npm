...
class Wrangler extends events.EventEmitter {
    ...
    create_gremlin_load_file() {
        ...
        // Edges for: maintainer 1 -> knows -> maintainer 2  (dedup these with linux 'uniq' command)
        // Edges for: maintainer 2 -> knows -> maintainer 1  (dedup these with linux 'uniq' command)
        // for (var i = 0; i < sorted_lib_maintainer_ids.length; i++) {
        //     var lib = sorted_lib_maintainer_ids[i];
        //     var mids = lib_maintainers[lib];
        //     for (var m1 = 0; m1 < mids.length; m1++) {
        //         var mid1 = mids[m1];
        //         for (var m2 = 0; m2 < mids.length; m2++) {
        //             var mid2 = mids[m2];
        //             if (mid1 !== mid2) {
        //                 var stmt = util.format("g.V(['MAINT-%s','MAINT-%s']).addE('knows').to(g.V(['MAINT-%s','MAINT-%s']))", mid1, mid1, mid2, mid2);
        //                 gremlin_stmts.push(stmt);
        //                 var stmt = util.format("g.V(['MAINT-%s','MAINT-%s']).addE('knows').to(g.V(['MAINT-%s','MAINT-%s']))", mid2, mid2, mid1, mid1);
        //                 gremlin_stmts.push(stmt);
        //             }
        //         }
        //     }
