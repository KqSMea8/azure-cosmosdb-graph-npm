...
// Chris Joakim, Microsoft, 2019/06/13
...
class Wrangler extends events.EventEmitter {
	...
	aggregate_lib_files() {
        ...
        this.augment_aggregated_libraries_with_usage_counts(agg_list);

        this.fsu.write_json_file(Constants.AGGREGATED_LIBRARIES_FILE, agg_list);
    }

    augment_aggregated_libraries_with_usage_counts(agg_list) {
        var usage_counter = {};
    ...
    increment_counter(counter, key) {
        //console.log('increment_counter: ' + key);
        if (key in counter) {
            var value = counter[key];
            counter[key] = Number(value + 1);
        }
        else {
            //console.log('increment_counter initialize: ' + key);
